...
describe('buildCsvRows', () => {
  ...
  it('includes assignments on the start date boundary', () => {
    const csv = buildCsvRows(
      [makeAssignment({ date: '2024-01-01' })],
      [makeWorker()],
      [makeShift()],
      [],
      '2024-01-01',
  ...
  it('includes assignments on the end date boundary', () => {
    const csv = buildCsvRows(
      [makeAssignment({ date: '2024-01-07' })],
      [makeWorker()],
      [makeShift()],
      [],
      '2024-01-07',
  ...
  it('outputs correct column values for a single assignment', () => {
    ...
    const rows = parseCsv(
      buildCsvRows([assignment], [worker], [shift], [], '2024-01-01', '2024-01-01'),
    )
    const [, data] = rows
  ...
  describe('day names', () => {
    ...
    it.each(cases)('date %s maps to %s', (date, expectedDay) => {
      const rows = parseCsv(
        buildCsvRows([makeAssignment({ date })], [makeWorker()], [makeShift()], [], date, date),
      )
  ...
  describe('sorting', () => {
    it('sorts by date ascending', () => {
      ...
      const rows = parseCsv(
        buildCsvRows(assignments, workers, [shift], [], '2024-01-01', '2024-01-02'),
      )
    ...
    it('sorts by shift start time within the same date', () => {
      ...
      const rows = parseCsv(
        buildCsvRows(assignments, [worker], shifts, [], '2024-01-01', '2024-01-01'),
      )
    ...
    it('sorts by worker name within same date and shift', () => {
      ...
      const rows = parseCsv(
        buildCsvRows(assignments, workers, [shift], [], '2024-01-01', '2024-01-01'),
      )
  ...
  describe('tag handling', () => {
    it('joins tag names with "; " sorted alphabetically', () => {
      ...
      const rows = parseCsv(
        buildCsvRows([makeAssignment()], [worker], [makeShift()], tags, '2024-01-01', '2024-01-01'),
      )
    ...
    it('ignores tag IDs that have no matching tag record', () => {
      ...
      const rows = parseCsv(
        buildCsvRows([makeAssignment()], [worker], [makeShift()], [], '2024-01-01', '2024-01-01'),
      )
      expect(rows[1][7]).toBe('')
    })
  })
  ...
  describe('data integrity', () => {
    ...
    it('handles empty notes without quoting', () => {
      ...
      const rows = parseCsv(
        buildCsvRows([assignment], [makeWorker()], [makeShift()], [], '2024-01-01', '2024-01-01'),
      )
...
describe('triggerPrint', () => {
  ...
  it('includes the date range heading in the HTML', () => {
    triggerPrint([], [], [], [], '2024-01-01', '2024-01-07')
    expect(fakeContainer.innerHTML).toContain('1 Jan 2024')
    expect(fakeContainer.innerHTML).toContain('7 Jan 2024')
  })
