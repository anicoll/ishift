...
export function buildCsvRows(
  ...
  const rows = assignments
    ...
    .filter(
      (x): x is { a: Assignment; worker: Worker; shift: ShiftType } =>
...
export function triggerPrint(
  ...
  const shiftRows = shifts
    .map((shift) => {
      const cells = days
        ...
        .join('')
      const tagNames = shift.requiredTagIds
        .map((id) => tagMap.get(id)?.name ?? '')
        .filter(Boolean)
        .join(', ')
  ...
  const startLabel = new Date(startDate + 'T00:00:00').toLocaleDateString('en-AU', {
    month: 'short',
    day: 'numeric',
    year: 'numeric',
  })
  const endLabel = new Date(endDate + 'T00:00:00').toLocaleDateString('en-AU', {
    month: 'short',
    day: 'numeric',
    year: 'numeric',
  })
