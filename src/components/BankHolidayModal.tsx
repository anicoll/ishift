...
const REGIONS: Region[] = [
  ...
  {
    country: 'US', countryName: 'United States', subdivisions: [],
  },
  {
    country: 'CA', countryName: 'Canada', subdivisions: [],
  },
];
...
export function BankHolidayModal({ open, bankHolidays, store, onClose }: Props) {
  ...
  const importableCount = fetched.filter(h => selected.has(h.date) && !existingDates.has(h.date)).length;
    ...
    <Modal title="Bank Holidays" open={open} onClose={onClose} size="lg">
      <div className="form">
        ...
        <div className="bh-section">
          ...
          {fetchError && (
            <p className="bh-fetch-error">{fetchError}</p>
          )}
            ...
            <>
              <div className="bh-preview-header">
                ...
                <div className="bh-preview-actions">
                  <button
                    className="btn btn--ghost btn--sm"
                    onClick={() => setSelected(new Set(fetched.filter(h => !existingDates.has(h.date)).map(h => h.date)))}
                  >
                    Select all new
                  </button>
                  <button
